...
class Launcher(ctk.CTk):
    def __init__(self):
        ...
        self.search_entry = ctk.CTkEntry(
            ...
            placeholder_text="Search a game..."
        )
        ...
        # ScrollableFrame catalogue (ONLY game cards)
        ...
        # RIGHT: installed games
        right = ctk.CTkFrame(content, width=260, corner_radius=10)
        right.pack(side="right", fill="y", padx=(6, 12), pady=10)
        ctk.CTkLabel(right, text="Installed games", font=self.font_subtitle).pack(pady=(10, 4))
        ...
        self.catalog = []
        self.cards = []
        self.set_status("Loading catalogue...")
    ...
    def load_catalogue(self):
        try:
            ...
            self.catalog = []
            self.after(0, lambda: messagebox.showerror("Error", f"Unable to load catalogue:\n{e}"))
        self.after(0, self.render_catalogue)

    def render_catalogue(self):
        # clear ONLY the scrollable content (not search bar)
        ...
        if not filtered_catalog:
            ctk.CTkLabel(self.left_scroll, text="No games found in catalogue.").pack(pady=12)
            return
        ...
        for game in filtered_catalog:
            ...
            btn = ctk.CTkButton(
                right,
                text="Launch" if folder.exists() else "Download",
                width=110,
        ...
        self.set_status(f"{len(filtered_catalog)} games loaded")
    ...
    def start_download(self, game, pbar):
        ...
        if dest.exists():
            return messagebox.showinfo("Info", "Already downloaded.")

        def cb(dl, total):
            val = dl / total if total else 0
            self.after(0, lambda: (pbar.set(val), self.global_progress.set(val)))
            self.set_status(f"Downloading {name}: {int(val * 100)}%")
    ...
    def update_btn_for(self, name):
        for c in self.cards:
            if c["game"].get("name") == name:
                folder = DOWNLOADS_DIR / sanitize_name(name)
                c["btn"].configure(text="Launch", command=lambda: self.launch_folder_or_exe(folder))
    ...
    def refresh_downloaded_list(self):
        ...
        for entry in DOWNLOADS_DIR.iterdir():
            ...
            ctk.CTkButton(
                ...
                text="Launch",
                width=90,
                command=lambda p=entry: self.launch_folder_or_exe(p)
    ...
    def reset_cache(self):
        for folder in [IMAGE_CACHE_DIR, DOWNLOADS_DIR]:
            try:
                ...
                messagebox.showerror("Error: Make sure everything is properly closed", f"Unable to reset {folder} : {e}")
        self.set_status("Cache and downloads cleared.")
        self.refresh_downloaded_list()
