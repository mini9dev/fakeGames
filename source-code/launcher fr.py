...
# ── Module Presence Discord ────────────────────────────────────────────────────
class DiscordRPC:
    """Gere la Rich Presence Discord de facon thread-safe."""
    ...
    def set_idle(self):
        """Presence : launcher ouvert, aucun jeu en cours."""
        self._update(
            state="Contourne le systeme d'orbs avec facilite ;)",
            details="FakeGames Launcher",
            large_image="fg-logo",
            large_text="v" + CURRENT_VERSION,
            buttons=[{"label": "Installer le launcher", "url": "https://github.com/mini9dev/fakeGames"}],
        )

    def set_playing(self, game_name: str):
        """Presence : un faux jeu est lance."""
        self._update(
            state=f"Fait semblant de jouer a {game_name} pour avoir plus d'orbs :P",
            details="FakeGames Launcher",
            large_image="fg-logo",
            large_text="v" + CURRENT_VERSION,
            buttons=[{"label": "Installer le launcher", "url": "https://github.com/mini9dev/fakeGames"}],
...
# Charger et appliquer le theme sauvegarde au demarrage
...
class CreateGameModal(ctk.CTkToplevel):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(
            form, text="Chemin ou nom du .exe du vrai jeu",
        ...
        ctk.CTkLabel(
            ...
            text_color=TEXT_MUTED, anchor="w", wraplength=480,
        ).pack(fill="x", pady=(0, 4))
        ctk.CTkEntry(
            form, textvariable=self.exe_name_var,
        ...
        ctk.CTkLabel(
            preview_row, text="Chemin final detecte :",
            font=ctk.CTkFont(size=10), text_color=TEXT_MUTED
        ).pack(side="left")
        self.preview_lbl = ctk.CTkLabel(
            preview_row, text="-",
            font=ctk.CTkFont(size=10, weight="bold"), text_color=ACCENT3
        )
        ...
        ctk.CTkLabel(
            ...
            text=(
                ...
                "Compilation : environ 30 a 60 secondes."
            ),
        ...
        self.build_btn = ctk.CTkButton(
            self, text="Compiler le faux jeu",
        ...
        self.log_label = ctk.CTkLabel(
            log_frame, text="En attente...",
            font=ctk.CTkFont(family="Consolas", size=10),
            text_color=TEXT_MUTED, anchor="w", justify="left", wraplength=490,
        )
    ...
    def _done(self, ok, path_or_err):
        ...
        if ok:
            self.after(0, lambda: self.log_label.configure(
                text="Succes ! Fermeture dans 2 secondes...", text_color=ACCENT2))
            self.after(0, lambda: self._on_success(path_or_err))
            self.after(2000, self._safe_close)
        else:
            self.after(0, lambda: self.log_label.configure(
                text=f"Erreur : {path_or_err}", text_color=RED_SOFT))
...
class AdModal(ctk.CTkToplevel):
    ...
    def _build_ui(self):
        ...
        ctk.CTkButton(
            hdr, text="X", width=34, height=26,
            fg_color="transparent", hover_color="#2a1520",
            text_color=TEXT_MUTED, command=self._cancel,
        ...
        ctk.CTkLabel(
            body, text=self._ad.get("title", "Sponsor"),
            font=ctk.CTkFont(size=18, weight="bold"), text_color=TEXT_PRIMARY,
        ).pack(anchor="w")
        ctk.CTkLabel(
            body, text=self._ad.get("text", ""),
        ...
        if sponsor_url:
            ctk.CTkButton(
                ...
                text_color=TEXT_PRIMARY, height=36, corner_radius=7,
        ...
        self.continue_canvas = tk.Canvas(
            actions, width=self.continue_width, height=self.continue_height,
            bg=BG_MODAL, highlightthickness=0, bd=0,
        )
    ...
    def _set_ad_placeholder(self):
        for child in self.image_slot.winfo_children():
            child.destroy()
        ctk.CTkLabel(
            self.image_slot, text="FAKEGAMES",
            font=ctk.CTkFont(family="Segoe UI Black", size=26, weight="bold"),
            text_color=ACCENT,
        ).pack(pady=(28, 2))
        ctk.CTkLabel(
            self.image_slot, text="Espace partenaire",
            font=ctk.CTkFont(size=12), text_color=TEXT_MUTED,
        ).pack(pady=(0, 28))
...
class GameCard(ctk.CTkFrame):
    def __init__(
        ...
        self.img_frame = ctk.CTkFrame(
            self, width=168, height=94, corner_radius=8,
    ...
    def _build_download_button(self):
        ...
        self.pbar = ctk.CTkProgressBar(
            self.btn_frame, width=115, height=6,
            progress_color=ACCENT, fg_color="#1e2240", corner_radius=3
        )
        self.pbar.set(0)
        self.pbar_label = ctk.CTkLabel(
            self.btn_frame, text="", font=ctk.CTkFont(size=10), text_color=TEXT_MUTED
        )
    ...
    def _retry_cb(self, attempt, max_retries):
        self.after(0, lambda: self.pbar_label.configure(
            text=f"Tentative {attempt}/{max_retries}...", text_color=ACCENT3))
...
class UpdateModal(ctk.CTkToplevel):
    ...
    def _build_ui(self, version):
        ...
        ctk.CTkLabel(body,
                     text=f"Version v{version} disponible  (actuelle : v{CURRENT_VERSION})",
                     font=ctk.CTkFont(size=12, weight="bold"),
                     text_color=TEXT_PRIMARY).pack(anchor="w", pady=(0, 4))
        ctk.CTkLabel(body,
                     text="Le setup va etre telecharge puis lance automatiquement.",
        ...
        if self._changelog:
            ...
            ctk.CTkLabel(cl, text="Nouveautes :",
                         font=ctk.CTkFont(size=11, weight="bold"),
            ...
            if len(self._changelog) > 8:
                ctk.CTkLabel(cl,
                             text=f"+ {len(self._changelog) - 8} autre(s) changement(s)",
                             font=ctk.CTkFont(size=10),
        ...
        self.update_btn = ctk.CTkButton(
            btn_row, text="Mettre a jour",
            fg_color=ACCENT2, hover_color=ACCENT2, text_color=BG_DARK,
            font=ctk.CTkFont(size=12, weight="bold"),
            height=36, corner_radius=7, command=self._start_download)
        self.update_btn.pack(side="left", padx=(0, 10))

        self.later_btn = ctk.CTkButton(
            btn_row, text="Plus tard",
    ...
    def _download_worker(self):
        ...
        try:
            ...
            self.after(0, lambda: self.status_lbl.configure(text="Connexion..."))
                ...
                with open(tmp_path, "wb") as fh:
                    for chunk in r.iter_content(chunk_size=65536):
                        ...
                        if total:
                            ...
                            self.after(0, lambda _p=p: self.status_lbl.configure(
                                text=f"Telechargement... {_p}%"))
                        else:
                            mb = downloaded / (1024 * 1024)
                            self.after(0, lambda _m=mb: self.status_lbl.configure(
                                text=f"Telechargement... {_m:.1f} Mo"))

            if self._cancelled:
                return

            self.after(0, lambda: self.pbar.set(1.0))
            self.after(0, lambda: self.status_lbl.configure(text="Pret — lancement du setup..."))
            self.after(600, lambda: self._launch_setup(tmp_path))

        except requests.exceptions.ConnectionError:
            self.after(0, lambda: self._on_error("Connexion impossible. Verifiez votre reseau."))
        except requests.exceptions.Timeout:
            self.after(0, lambda: self._on_error("Delai depasse. Reessayez ou ouvrez GitHub."))
        except requests.exceptions.HTTPError as e:
            code = e.response.status_code if e.response else "?"
            self.after(0, lambda c=code: self._on_error(f"Erreur HTTP {c}"))
        except Exception as e:
    ...
    def _on_error(self, err: str):
        print(f"[UpdateModal] Erreur telechargement : {err}")
        self.status_lbl.configure(text=f"Erreur : {err[:90]}")
        self.update_btn.configure(
            state="normal", text="Reessayer",
            fg_color=ACCENT2, hover_color=ACCENT2,
            text_color=BG_DARK, command=self._start_download)
...
class Launcher(ctk.CTk):
    ...
    def _empty_state(self, title, subtitle, accent=ACCENT):
        ...
        ctk.CTkLabel(panel, text=title,
                     font=ctk.CTkFont(size=16, weight="bold"), text_color=TEXT_PRIMARY).pack(padx=46)
        ctk.CTkLabel(
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(sidebar, text=f"v{CURRENT_VERSION} - FakeGames Launcher",
                     font=ctk.CTkFont(size=9), text_color=TEXT_MUTED).pack(side="bottom", pady=(0, 4))
        self.disk_lbl = ctk.CTkLabel(sidebar, text="0 Mo utilise",
                                     font=ctk.CTkFont(size=10), text_color=TEXT_MUTED)
        self.disk_lbl.pack(side="bottom", pady=(0, 2))

        self.scroll = ctk.CTkScrollableFrame(
            body, fg_color=BG_DARK,
    ...
    def _render_catalogue(self):
        ...
        if not filtered:
            self._empty_state(
                "Aucun jeu trouve",
                "Modifiez la recherche ou remettez le tri par defaut pour revoir tout le catalogue.",
            )
            return
    ...
    def _render_custom(self):
        ...
        if not custom_games:
            self._empty_state(
                ...
                "Creez votre premier faux jeu depuis le bouton orange de la barre laterale.",
                accent=ACCENT3,
            )
            return
    ...
    def _render_settings(self):
        ...
        def _on_theme_select(name):
            ...
            s["theme"] = name
            save_settings(s)
            self.toast(f"Theme '{name}' applique — relancez l'app pour l'effet complet.", ACCENT)
            self._render_settings()

        ctk.CTkLabel(self.scroll, text="Parametres",
                     font=ctk.CTkFont(family="Segoe UI Black", size=20, weight="bold"),
                     text_color=ACCENT).pack(anchor="w", padx=32, pady=(24, 4))

        _section("Apparence — Theme visuel")

        for name, desc, acc in [
            ("Dark",   "Fond sombre, accent bleu",   "#5865f2"),
            ("Light",  "Fond clair, texte fonce",     "#5865f2"),
            ("Purple", "Fond sombre, accent violet",  "#9b59f5"),
        ]:
            ...
            if is_active:
                ctk.CTkLabel(row, text="✓ Actif",
                             font=ctk.CTkFont(size=11, weight="bold"),
                             text_color=acc).pack(side="right", padx=14)
            else:
                ctk.CTkButton(row, text="Choisir", width=80, height=28,
        ...
        # Icone Discord (cercle violet)
        ctk.CTkFrame(discord_row, width=12, height=12, fg_color="#5865f2",
                     corner_radius=6).pack(side="left", padx=(14, 10), pady=16)
        ctk.CTkLabel(discord_row, text="Afficher le statut sur Discord",
                     font=ctk.CTkFont(size=12, weight="bold"),
                     text_color=TEXT_PRIMARY).pack(side="left")
        ctk.CTkLabel(discord_row, text="  —  Rich Presence dans votre profil",
        ...
        def _toggle_discord():
            ...
            if new_val:
                _discord_rpc.connect()
                self.toast("Presence Discord activee", "#5865f2")
            else:
                _discord_rpc.disconnect()
                self.toast("Presence Discord desactivee", TEXT_MUTED)
            self._render_settings()

        toggle_text = "✓ Active" if discord_enabled else "Activer"
        ...
        ctk.CTkButton(discord_row, text=toggle_text, width=90, height=28,
                      ...
                      border_color=BORDER, text_color=TEXT_PRIMARY if discord_enabled else TEXT_MUTED,
                      font=ctk.CTkFont(size=11),
                      command=_toggle_discord,
                      ).pack(side="right", padx=14, pady=8)

        _section("Zone de danger", color=RED_SOFT)

        ctk.CTkLabel(self.scroll,
                     text="Supprime tous les jeux telecharges, les jeux custom et le cache d'images.",
                     font=ctk.CTkFont(size=11), text_color=TEXT_MUTED,
                     wraplength=600, justify="left").pack(anchor="w", padx=32, pady=(0, 8))

        ctk.CTkButton(self.scroll, text="Reset cache & jeux",
                      fg_color="transparent", hover_color="#2a1520",
                      border_width=1, border_color=RED_SOFT, text_color=RED_SOFT,
                      font=ctk.CTkFont(size=12), height=36, corner_radius=7,
    ...
    def _start_download(self, game, progress_cb, retry_cb, done_cb):
        ...
        def _ui_retry(attempt, max_retries):
            self.after(0, lambda: self.set_status(
                f"Timeout - tentative {attempt}/{max_retries}...", ACCENT3))
            self.after(0, lambda: self.toast(
                f"Connexion lente - tentative {attempt}/{max_retries}", ACCENT3))
    ...
    def _update_counts(self, filtered_count=None):
        ...
        if filtered_count is not None:
            ...
            self.count_lbl.configure(text="Parametres")
    ...
    def _finish_disk_refresh(self, mb):
        ...
        self.disk_lbl.configure(
            text=f"{mb / 1024:.2f} Go utilise" if mb >= 1024 else f"{mb:.1f} Mo utilise")
        ...
        def worker():
            for folder in (IMAGE_CACHE_DIR, DOWNLOADS_DIR, CUSTOM_DIR):
                try:
                    ...
                    self.after(0, lambda: self.toast(
                        "Reset impossible. Fermez tous les jeux ouverts.", RED_SOFT))
